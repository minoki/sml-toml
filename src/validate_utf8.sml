...
structure ValidateUtf8 :> VALIDATE_UTF8 = struct
  exception InvalidUtf8
   ...
   *)
  fun nextState (START, c) = if c < #"\u0080" then
                                 START
                             else if c < #"\u00E0" then
                                 if #"\u00C2" <= c then
                                     MID_1_OF_2
                                 ...
                                 raise InvalidUtf8
    | nextState (MID_1_OF_2, c1) = if #"\u0080" <= c1 andalso c1 < #"\u00C0" then
                                       START
                                   else
                                       raise InvalidUtf8
    | nextState (MID_1_OF_3_E0, c1) = if #"\u0080" <= c1 andalso c1 < #"\u00C0" andalso #"\u00A0" <= c1 then
                                          MID_2_OF_3
                                      else
                                          raise InvalidUtf8
    | nextState (MID_1_OF_3_ED, c1) = if #"\u0080" <= c1 andalso c1 < #"\u00C0" andalso c1 < #"\u00A0" then
                                          MID_2_OF_3
                                      else
                                          raise InvalidUtf8
    | nextState (MID_1_OF_3_OTHER, c1) = if #"\u0080" <= c1 andalso c1 < #"\u00C0" then
                                             MID_2_OF_3
                                         else
                                             raise InvalidUtf8
    | nextState (MID_2_OF_3, c2) = if #"\u0080" <= c2 andalso c2 < #"\u00C0" then
                                       START
                                   else
                                       raise InvalidUtf8
    | nextState (MID_1_OF_4_F0, c1) = if #"\u0080" <= c1 andalso c1 < #"\u00C0" andalso #"\u0090" <= c1 then
                                          MID_2_OF_4
                                      else
                                          raise InvalidUtf8
    | nextState (MID_1_OF_4_OTHER, c1) = if #"\u0080" <= c1 andalso c1 < #"\u00C0" then
                                             MID_2_OF_4
                                         else
                                             raise InvalidUtf8
    | nextState (MID_1_OF_4_F4, c1) = if #"\u0080" <= c1 andalso c1 < #"\u00C0" andalso c1 < #"\u0090" then
                                          MID_2_OF_4
                                      else
                                          raise InvalidUtf8
    | nextState (MID_2_OF_4, c2) = if #"\u0080" <= c2 andalso c2 < #"\u00C0" then
                                       MID_3_OF_4
                                   else
                                       raise InvalidUtf8
    | nextState (MID_3_OF_4, c3) = if #"\u0080" <= c3 andalso c3 < #"\u00C0" then
                                       START
                                   else
                                       raise InvalidUtf8
  fun go (START, NONE) = NONE
    | go (_, NONE) = raise InvalidUtf8
    | go (state, SOME (c, strm)) = SOME (c, (strm, nextState (state, c)))
  fun validatingReader getc (strm, state) = go (state, getc strm)
end
