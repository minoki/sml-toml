...
functor ParseToml (Handler : TOML_VALUE_HANDLER) :> PARSE_TOML
  ...
  fun UnexpectedEndOfInput expected = raise E.ParseError (E.UNEXPECTED { encountered = "end of input", expected = expected })
  fun UnexpectedChar (c, expected) = raise E.ParseError (E.UNEXPECTED { encountered = Char.toString c, expected = expected })
  fun DuplicateKey path = raise E.ParseError (E.DUPLICATE_KEY path)
  (*: val parse : (char, 'strm) StringCvt.reader -> 'strm -> table *)
    ...
    = let (*: val expect : char * 'strm -> 'strm *)
          fun expect (c, strm) = case getc strm of
                                   ...
                                   | SOME (c', strm') => if c = c' then
                                                             ...
                                                             UnexpectedChar (c', Char.toString c)
            ...
            fun go (accum, strm) = case getc strm of
                                     ...
                                     | SOME (#"\\", strm') => (case getc strm' of
                                                                 ...
                                                                 | SOME (#"U", strm'') => let val (hi, strm''') = readFourHexDigit strm''
                                                                                          ...
                                                                                          in if hi > 0x0010 then
                                                                                                 raise E.ParseError E.INVALID_UNICODE_SCALAR
                                                                                             else
                                                                                                 ()
                                                                                           ; go (revAppendUtf8 (Word.fromInt (hi * 65536 + lo), accum), strm'''')
                                                                                          end
          ...
          and readHexIntUnderscore (accum, strm) = case getc strm of
                                                       NONE => (accum, strm)
                                                     | SOME (#"_", strm') => readHexInt (accum, strm') (* the next character must be a hexadecimal digit *)
                                                     | SOME (c, strm') => if #"0" <= c andalso c <= #"9" then
                                                                              readHexIntUnderscore (Handler.Integer.+ (Handler.Integer.* (accum, Handler.Integer.fromInt 16), Handler.Integer.fromInt (digitToInt c)), strm')
                                                                          else if #"A" <= c andalso c <= #"F" then
                                                                              readHexIntUnderscore (Handler.Integer.+ (Handler.Integer.* (accum, Handler.Integer.fromInt 16), Handler.Integer.fromInt (Char.ord c - (Char.ord #"A" - 10))), strm')
          ...
          fun readSigned (sign, d0, strm) = let val (accum, _, strm') = readDecIntUnderscore ([d0, sign], false, strm)
                                                fun checkPrefixZero () = case accum of
                                                                            [_, _] => ()
                                                                          | _ => if d0 = #"0" then
                                                                                      raise E.ParseError E.PREFIX_ZERO
                                                                                  else
                                                                                      ()
          ...
          in
            ...
            fun readMinSec (accum, strm) = let val (accum', min, strm') = readTwoDigit (accum, strm) (* minute *)
                                           ...
                                           in if min < 60 andalso sec <= 60 then
                                                  ...
                                                  raise E.ParseError E.INVALID_TIME
                                            ; case getc strm''' of
          ...
          fun readDate (accum, year, strm) = let val (accum', month, strm') = readTwoDigit (accum, strm) (* month *)
                                                 ...
                                                 val () = if isValidDate (year, month, mday) then
                                                              ...
                                                              raise E.ParseError E.INVALID_DATE
                                             in case getc strm''' of
            ...
            | readSimpleKey (SOME (c0, strm))
                = let fun go (accum, strm') = case getc strm' of
                  ...
                  in if isValidUnquotedKey c0 then
                         ...
                         UnexpectedChar (c0, "quote, alphanum, '-', or '_'")
                  end
                        ...
                        | go (accum, (p as (key', pv)) :: xs)
                            = if key = key' then
                                  case pv of
                                    ...
                                    | PARTIAL_TABLE (EXACT_HEADER, _) => DuplicateKey (List.revAppend (revPath, [key]))
                                    | PARTIAL_TABLE (definedBy, pt') =>
                                      List.revAppend (accum, (key', PARTIAL_TABLE (definedBy, insert (key :: revPath, pt', keys, v))) :: xs)
                                    | PARTIAL_ARRAY _ => DuplicateKey (List.revAppend (revPath, [key]))
                              else
                                  go (p :: accum, xs)
                  in go ([], pt)
                  end
          ...
          *)
                    ...
                    | go (accum, (p as (key', pv)) :: xs)
                        = if key = key' then
                              case pv of
                                ...
                                | PARTIAL_TABLE (_, _) => DuplicateKey (List.revAppend (revPath, [key]))
                                | PARTIAL_ARRAY _ => DuplicateKey (List.revAppend (revPath, [key])) (* previous: array, now: table *)
                          else
                              go (p :: accum, xs)
              in go ([], pt)
              end
            ...
            = let fun go (accum, []) = List.revAppend (accum, [(key, PARTIAL_ARRAY ([], []))]) (* new array table *)
                    | go (accum, (p as (key', pv)) :: xs)
                        = if key = key' then
                              case pv of
                                  LEAF _ => DuplicateKey (List.revAppend (revPath, [key]))
                                | PARTIAL_TABLE _ => DuplicateKey (List.revAppend (revPath, [key]))
          ...
          fun parse (accum, revPath, prefix, strm)
            = case readExpression (revPath, strm) of
                ...
                | SOME (KEYVAL (key, v), strm') => parse (insertKeyval ([], accum, prefix, key, v), revPath, prefix, strm')
